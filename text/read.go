...
func (rb readByteChunksImpl)Step() (io.IO[stream.StepResult[[]byte]]) {
	return io.Eval(func() (res stream.StepResult[[]byte], err error){
		bytes := make([]byte, rb.chunkSize)
		var cnt int
		cnt, err = rb.reader.Read(bytes)
		if err == fio.EOF {
			err = nil
			res = stream.StepResult[[]byte]{
				Continuation: stream.Empty[[]byte](),
				HasValue: false,
			}
		} else if err == nil {
			if cnt == 0 {
...
func (rb readByteChunksImpl)IsFinished() io.IO[bool] { return io.Lift(false) }
...
func MapToStrings(stm stream.Stream[[]byte]) stream.Stream[string] {
	return stream.MapPure(stm, func(a []byte) string {return string(a)})
}
